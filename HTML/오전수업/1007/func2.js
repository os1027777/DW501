/*
  ...
  4. number_input함수에서 세 개의 input 값 가져오기
  5. number_input함수에서 number_check함수 실행
  6. number_check함수에 input값 3개 전달
  7. number_input함수에서 input 값과 배열과 비교하기
...
function input_check(user) {
  for (var i in user) {
    if (user[i] < 1 || user[i] > 9) return true;
  }
...
function print(count) {
  ...
  div.appendChild(document.createTextNode(count + "개 일치"));
  res.prepend(div);
}
